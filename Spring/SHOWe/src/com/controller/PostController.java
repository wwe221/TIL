...
public class PostController {
	...
	public String postcommentadd(Post post ,String postid) {
		// 댓글 작성
		int cat = Integer.parseInt(postid);
	...
	public ModelAndView postdetail(ModelAndView mv, String id) {
		...
		ArrayList<Integer> comId = new ArrayList<Integer>();
		ArrayList<Post> recomments = null;
		...
		try {
			...
			recomments = pbiz.select();
			for (Post tmp : comments)
				comId.add(tmp.getId());
			for (int i = 0; i < recomments.size(); i++) {
				boolean flag = false;
				for (int j = 0; j < comId.size(); j++) {
					if (recomments.get(i).getReid() == comId.get(j)) {
						flag = true;
					}
				}
				if (!flag)
