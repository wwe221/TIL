...
<div class="center_page">
	<h3 class="boardtitle">
		<i class="fas fa-book-open"></i>&nbsp;게시글 보기
	</h3>
			...
			<tr>
				<td class="detail_review">
					<table>
						<tbody>
							<tr valign="top">
								<td><span class="post_title"> ${postdetail.title} </span></td>
							...
							<tr>
								<td><c:choose>
										<c:when test="${postdetail.writer == loginuser.id}">
											...
											<a
												href="postdelete.sh?id=${postdetail.id }&cate=${postdetail.category}">DELETE</a>
										</c:when>
									</c:choose></td>
							</tr>
						</tbody>
		...
		<c:forEach var="comm" items="${comments}">
			<li>
				...
				<div id="${comm.id}ori" class="origin_text">
					<p>${comm.text}</p>
					<button onclick="myShowFunction('${comm.id}re')">답변달기</button>
					<div class="recommBox" id="${comm.id}rediv">
					${recomm.id }
					<form action="postcommentadd.sh" method="POST">
						...
						<td><input type="hidden" name="title" value="comment">
							...
							type="hidden" name="reid" value="${comm.id}"> <input
							type="hidden" name="contents" value="1"> <input
							type="hidden" name="writer" value="${loginuser.id}"> 
			...
			<c:forEach var="recomm" items="${recomments}">
				<c:choose>
					<c:when test="${recomm.reid == comm.id}">
						<li class="recomments">
							<div class="com_list_each">${recomm.writer}/${recomm.regdate}</div>
							<div id="${recomm.id}ori" class="origin_text">
								<p>${recomm.text}</p>
							</div>
						</li>
					</c:when>
